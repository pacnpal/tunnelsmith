...
Phase 11 shipped the control listener with no credential check; the security boundary was the network (loopback or private subnet). v1.1.0 adds an opt-in bearer-token gate so multi-tenant or LAN-exposed deployments can gate `POST /v1/report` without breaking the no-auth wire shape. `[control].auth_tokens` (inline) and `[control].auth_tokens_file` (one token per line, `#` comments, dedup'd union) configure a token set; an empty set keeps Phase 11 behavior byte-for-byte. When non-empty, requests must carry `Authorization: Bearer <token>`; 401 responses include `WWW-Authenticate: Bearer realm="tunnelsmith"` plus RFC 6750 §3 `Cache-Control: no-store` and `Pragma: no-cache`. The auth check runs `crypto/subtle.ConstantTimeCompare` across the live token set without short-circuiting on match, so per-request work is independent of which token matched. Each handler captures a single `TokenSnapshot` at the start of the request and runs every auth question (`Enabled`, `Allow`) off that view so a concurrent SIGHUP rotation cannot tear the decision. Two new reject reasons (`auth_missing`, `auth_failed`) join `tunnelsmith_reports_rejected_total{reason}`. The Go SDK gained `client.Options.Token` for the corresponding client side. Rationale and the threat model (plaintext tokens on the listener; TLS termination is a Phase 13 candidate) are in [`ADR-007`](decisions.md).

## Pluggable upstream-pool providers (Phase 13)

v1.0.0 hard-coded the Mullvad adapter into `cmd/tunnelsmith` via an `expandMullvadPool` helper, with the provider enum gating valid `[[upstream_pool]].provider` values inside `internal/config`. v1.2.0 inverts that: `internal/upstream/provider` defines three small interfaces — `Provider` (a name and an `Expander`), `Expander` (`Lookup(ctx, cfg) → []Upstream`), and an optional `API` (vendor-side actions like proxy-list refresh) — plus a registry. Each provider package (`internal/upstream/mullvad`, `internal/upstream/webshare`) registers itself in `init()`; `cmd/tunnelsmith` dispatches through `provider.Default().Lookup` and never references a concrete provider type. The hard-coded enum in `internal/config` is replaced by a registry-backed validator (`config.SetProviderValidator`). Adding a provider is one new package plus a blank import line in `internal/upstream/providers/registry.go`. The Webshare adapter that ships in v1.2.0 expands a Webshare plan into one HTTP-with-Basic-auth or SOCKS5 upstream per active proxy, paginates `GET /api/v2/proxy/list/` with a hard cap of 500 pages, drops `valid = false` entries, and falls back to a disk-cached last-known-good list when the vendor API is unreachable. For providers that expose an API, two new control-listener routes — `GET /v1/providers` and `POST /v1/providers/{id_prefix}/refresh` — share the Phase 12 bearer-token gate; vendor errors map to `429` / `504` / `502` by category with a sanitised JSON body, and the full vendor error chain stays in the operator log only. The full surface and adapter-author flow are in [`docs/providers.md`](providers.md) and [ADR-008](decisions.md).

## Opt-in TLS on the control listener (Phase 14)

Phase 12 shipped bearer-token auth on the control listener but acknowledged in ADR-007's Non-goals that the tokens still travel in cleartext over whatever network segment carries the connection. v1.2.0 closes that gap with two new keys on `[control]` — `tls_cert_file` and `tls_key_file` (both-or-neither; absolute paths only) — that switch the listener from `http.Server.Serve` to `http.Server.ServeTLS`. Both empty preserves the Phase 11/12 plaintext wire shape byte-for-byte; both set terminates HTTPS on the same port with `MinVersion: tls.VersionTLS12` pinned on the underlying `tls.Config` so the policy survives any future stdlib default flip. Only one set is a config-load error so a half-configured deployment cannot silently fall back to plaintext, and `Serve` re-checks the invariant at startup for callers that bypass `config.Validate`. The keypair is pre-loaded with `tls.LoadX509KeyPair` before `net.Listen`, so a bad cert/key path surfaces without leaving an orphaned TCP socket bound on the control address. Cert rotation is restart-only in v1.2 — file watching / `GetCertificate` is deferred until an operator asks. Rationale, the pinned-baseline policy, and the explicit non-goals (mTLS, ACME, operator-tunable minimum version / cipher pinning, TLS on the metrics or UI listeners) are in [ADR-009](decisions.md).

## What v1.2.0 deliberately does not have
...
- mTLS / client-cert auth on the control listener. Bearer tokens over TLS (Phase 12 + Phase 14) cover the v1.2 threat model; client-cert auth and ACME / `GetCertificate` hot-reload are explicit non-goals in [ADR-009](decisions.md) and land in a follow-up if a workload demands them.
- TLS on the metrics or UI listeners. The same pattern would apply but is out of scope for 1.2; operators who want it can front those listeners with a reverse proxy in the meantime.
