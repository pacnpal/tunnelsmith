...
| key                          | type             | default       | notes |
|------------------------------|------------------|---------------|-------|
| `connection_refused`         | bool             | `true`        | when `false`, dial-side `ECONNREFUSED` errors are not scored: the upstream's penalty and cooldown are unchanged. The failed dial is still logged and counted in metrics; only the scoreboard penalty/cooldown step is skipped. Hot-reload (SIGHUP) picks up a changed value live |
