...
// upstream id (static or generated by another pool) would be routed
// into the wrong auto-heal driver by the strings.HasPrefix dispatch in
// Observe.
//
// Two collision shapes are caught:
//
//  1. Static [[upstream]] id starts with an auto-heal pool's
...
//
//  2. Two [[upstream_pool]] blocks where one's id_prefix is a
...
func validateNoPoolPrefixCollision(static []config.UpstreamConfig, drivers []*authHealDriver) error {
	for _, d := range drivers {
		prefix := d.idPrefix + "-"
	...
	// Pairwise pool-vs-pool: A's matcher "A-" must not be a prefix of
	// B's matcher "B-" for any A != B. Equivalently: B's id_prefix
	// must not start with A's id_prefix + "-".
	for i, a := range drivers {
		aMatcher := a.idPrefix + "-"
		for j, b := range drivers {
			if i == j {
				continue
			}
			bMatcher := b.idPrefix + "-"
			if strings.HasPrefix(bMatcher, aMatcher) {
				return fmt.Errorf(
					"auth-heal driver: [[upstream_pool]] id_prefix %q is a sub-prefix of id_prefix %q (events for the longer pool would also fire heals on the shorter one); rename one of the pools so neither id_prefix starts with the other followed by '-'",
